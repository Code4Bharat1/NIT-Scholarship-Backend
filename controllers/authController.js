...
export const register = async (req, res) => {
  try {
    ...
    // ✅ Check karo real User collection mein
    ...
    // ✅ Purana pending record hata do (agar same email/phone se dobara try kar raha ho)
    await PendingRegistration.deleteOne({ $or: [{ email }, { phone }] });

    // ✅ Sirf PendingRegistration mein save karo — User DB mein nahi
...
export const verifyEmail = async (req, res) => {
  try {
    ...
    // ✅ Pending collection mein dhundo
    ...
    // ✅ Agar SMS bhi verify ho chuka hai toh User create karo
    if (pending.isSmsVerified) {
      await createVerifiedUser(pending);
      return res.status(200).json({
        success: true,
        message: "Email verified! Registration completed.",
        data: { emailVerified: true, smsVerified: true, fullyRegistered: true },
      });
    }
...
export const verifySMS = async (req, res) => {
  try {
    ...
    // ✅ Agar Email bhi verify ho chuka hai toh User create karo
...
export const resendOTP = async (req, res) => {
  try {
    ...
    // ✅ Pehle pending mein dhundo, phir user mein
    const pending = await PendingRegistration.findOne({ email });
...
async function createVerifiedUser(pending) {
  // Final check — kahi duplicate toh nahi bana
  ...
  // ✅ Pending record hata do
  await PendingRegistration.deleteOne({ _id: pending._id });

  // ✅ Admit card + credentials emails bhejo
